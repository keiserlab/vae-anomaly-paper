...
class RandomMolecularStringGenerator:
    ...
    def generate_mutation_random(self,
                                 num_to_generate: int,
                                 n_bits: int) -> List[str]:
        ...
        while len(generated) < num_to_generate:
            for row in self.conn.execute(f"SELECT * FROM dataset USING SAMPLE {num_to_generate}").fetchall():
                row_tokenized = self.tokenizer.tokenize(row[0])
                if len(row_tokenized) >= n_bits:
                    # generate random indices to mutate
                    mutate_idxs = random.sample(range(len(row_tokenized)), n_bits)
                    for m_id in mutate_idxs:
                        mutation = random.choice(list(filter(lambda t: t != row_tokenized[m_id], self.token_vocab_lst)))
                        row_tokenized[m_id] = mutation
                generated.append(''.join(row_tokenized))

        return generated[:num_to_generate]
    
    def full_mutation_random(self, num_to_generate):
        generated = []
        for row in self.conn.execute(f"SELECT * FROM dataset USING SAMPLE {num_to_generate}").fetchall():
            row_tokenized = self.tokenizer.tokenize(row[0])
            n_bits = len(row_tokenized)
            mutate_idxs = random.sample(range(len(row_tokenized)), n_bits)
            for m_id in mutate_idxs:
                mutation = random.choice(list(filter(lambda t: t != row_tokenized[m_id], self.token_vocab_lst)))
                row_tokenized[m_id] = mutation
